...
void CPUOperation::free_memory(vector<float> *data) { delete data; }
...
// implement gpu operations so that the code compiles
// will be replaced by actual gpu operations if compiled with gpu support
...
vector<float> *__attribute__((weak))
GPUOperation::move_data(vector<float> *data) {
    throw_gpu_error();
    return nullptr;
}

void __attribute__((weak)) GPUOperation::free_memory(vector<float> *data) {
    throw_gpu_error();
}
