...
Tensor::Tensor(Buffer *data, string device, bool requires_grad) {
    ...
    if (requires_grad) {
        this->grad = new Buffer(new vector<float>(shape[0] * shape[1], 0),
                                device, shape);
    }
...
Tensor::Tensor(vector<vector<float>> *data, string device, bool requires_grad) {
    ...
    if (requires_grad) {
        grad = new Buffer(new vector<float>(shape[0] * shape[1], 0), device,
                          shape);
    }
...
void print_tensor(vector<float> *data, vector<std::size_t> shape,
                  string device) {
